...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
    ...
    // private PhotonPoseEstimator camPoseEstimator = new PhotonPoseEstimator(
    // layout, PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR,
    // Constants.PhotonVisionConstants.ROBOT_TO_CAMERA); // TODO Jerry, test
    // different pose strageies. In general just use multitag
    ...
    private void configureAutoBuilder() {
        try {
            ...
            AutoBuilder.configure(
                    ...
                    // DriverStation.getAlliance().get().equals(Alliance.Red))/*DriverStation.getAlliance().orElse(Alliance.Blue)
                    // == Alliance.Red*/,
    ...
    public void periodic() {
        ...
        // SmartDashboard.putNumber("Pose Y", getState().Pose.getY());
        // _field.setRobotPose(getState().Pose.getX(), getState().Pose.getY(),
        // getState().Pose.getRotation());

        // VISION

        // var visionEstimate = cam.getEst
        ...
        if (!results.isEmpty()) {
            ...
            if (cur.hasTargets()) {
                ...
                // if (bestResult != null) {
                // SmartDashboard.putNumber("Camera Yaw", bestResult.getYaw());
                // }
            ...
            // can potentially run through every result rather than just the most recent for
            // more accurate representation of data
            // var photonEstimate = camPoseEstimator.update(cur).orElse(null);
            // if( photonEstimate != null){
            // System.err.println("/////////" + " " +
            // photonEstimate.estimatedPose.toPose2d().getX() + " " +
            // photonEstimate.estimatedPose.toPose2d().getY() + " " +
            // photonEstimate.timestampSeconds);
            // // addVisionMeasurement(photonEstimate.estimatedPose.toPose2d(),
            // photonEstimate.timestampSeconds);
            // addVisionMeasurement(photonEstimate.estimatedPose.toPose2d(),
            // photonEstimate.timestampSeconds);
            // }
            ...
            // var visionMeasure =
            // camPose.transformBy(Constants.PhotonVisionConstants.CAMERA_TO_ROBOT);
            ...
            // var visionMeasure =
            // camPose.transformBy(Constants.PhotonVisionConstants.CAMERA_TO_ROBOT);
            // addVisionMeasurement(visionMeasure.toPose2d(),
            // Units.millisecondsToSeconds((double)timestamp/1000));
            // }
